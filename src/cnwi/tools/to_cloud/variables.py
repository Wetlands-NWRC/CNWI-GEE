...
def fourier_transform_2_cloud(grid: gpd.GeoDataFrame, bucket: str, file_prefix: str, filename: str = None) -> None:
    ...
    for id in grid_ids:
        ...
        task = ee.batch.Export.image.toCloudStorage(
            ...
            description="",
            bucket=bucket,
            fileNamePrefix=file_prefix + f'/{filename if filename is not None else "fourier-export"}/{id}/',
            scale=10,
            crs='EPSG:4326',
            region=region,
            maxPixels=1e13,
...
def terrain_analysis_2_cloud(grid: gpd.GeoDataFrame, bucket: str, file_prefix: str, filename: str = None):
    ...
    for grid_id in grid_ids:
        ...
        task = ee.batch.Export.image.toCloudStorage(
            ...
            description="",
            bucket=bucket,
            fileNamePrefix=file_prefix + f'/{filename if filename is not None else "terrain-export"}/{id}/',
            scale=30,
            fileDimensions=[4096, 4096],
            region=geom,
            crs='EPSG:4326',
